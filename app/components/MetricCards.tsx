...
export default function MetricCards({
  ...
  const changeColor = isUp === null ? 'text-slate-200' : isUp ? 'text-emerald-300' : 'text-red-300';
  const changeBg = isUp === null ? 'bg-white/5' : isUp ? 'bg-emerald-400/10' : 'bg-red-400/10';
  const changeBorder = isUp === null ? 'border-white/10' : isUp ? 'border-emerald-400/25' : 'border-red-400/25';
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
        ...
        <div className="mt-2 flex items-baseline justify-between gap-3">
          ...
          <div
            className={
              `rounded-xl border px-3 py-1 text-sm font-semibold ${changeColor} ${changeBg} ${changeBorder}`
            }
          >
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
        ...
        <div className="mt-2 text-3xl font-bold tracking-tight">{typeof sentimentScore === 'number' ? sentimentScore.toFixed(1) : '—'}</div>
